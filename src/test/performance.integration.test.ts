...
suite('Performance Integration Test Suite', () => {
	...
	test('DuckDB Service direct performance test', async function() {
		...
		try {
			...
			// Query first page (LIMIT 100)
			const query1Start = performance.now();
			const result1 = await service.executeQuery(
				`SELECT * FROM read_gdx('${registrationName}', '${testSymbol.name}') LIMIT 100 OFFSET 0`
			);
			const query1End = performance.now();
			console.log(`[PERF TEST] First page query (100 rows): ${(query1End - query1Start).toFixed(0)}ms`);
			...
			const result2 = await service.executeQuery(
				`SELECT * FROM read_gdx('${registrationName}', '${testSymbol.name}') LIMIT 100 OFFSET 100`
			);
			const query2End = performance.now();
			console.log(`[PERF TEST] Second page query (100 rows): ${(query2End - query2Start).toFixed(0)}ms`);
			
			// Query third page
			const query3Start = performance.now();
			const result3 = await service.executeQuery(
				`SELECT * FROM read_gdx('${registrationName}', '${testSymbol.name}') LIMIT 100 OFFSET 200`
			);
			const query3End = performance.now();
			console.log(`[PERF TEST] Third page query (100 rows): ${(query3End - query3Start).toFixed(0)}ms`);
			...
			// Verify row counts
			assert.strictEqual(result1.rowCount, 100, 'First page should have 100 rows');
			assert.strictEqual(result2.rowCount, 100, 'Second page should have 100 rows');
			assert.strictEqual(result3.rowCount, 100, 'Third page should have 100 rows');
